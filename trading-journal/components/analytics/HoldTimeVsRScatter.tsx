...
export function HoldTimeVsRScatter({ data }: HoldTimeVsRScatterProps) {
  ...
  if (data.length === 0) {
    return (
      <div className="flex h-64 items-center justify-center rounded-lg border border-slate-200/70 bg-white/80 p-6 dark:border-slate-800/60 dark:bg-slate-900/60">
  ...
  return (
    <div className="space-y-4 rounded-lg border border-slate-200/70 bg-white/80 p-6 dark:border-slate-800/60 dark:bg-slate-900/60">
      ...
      <div className="relative h-64 border border-slate-200 bg-slate-50 dark:border-slate-700 dark:bg-slate-900/40">
        {/* Zero R line */}
        <div className="absolute inset-x-0 top-1/2 border-t border-dashed border-slate-400 dark:border-slate-600" />

        {/* Y axis */}
        <div className="absolute inset-y-0 left-0 flex w-12 flex-col justify-between py-2 text-xs text-slate-600 dark:text-slate-400">
          ...
          {data.map((point, i) => {
            ...
            return (
              <div
                key={i}
                className={cn(
                  'absolute h-2 w-2 -translate-x-1 -translate-y-1 rounded-full transition-all hover:scale-150',
                  point.r > 0
        ...
        {/* X axis */}
        <div className="absolute inset-x-12 bottom-0 flex justify-between pb-1 text-xs text-slate-600 dark:text-slate-400">
          <span>0m</span>
      ...
      {hoveredPoint && (
        <div className="rounded border border-slate-200 bg-slate-100 p-3 text-xs dark:border-slate-700 dark:bg-slate-800">
